...
##### merged argon1, argon1v1, argon1v5, argoneon, argonfanhat setup script
...
do
	...
	then
		loopflag=0
		if [[ $newmode -eq 2 || $newmode -eq 5 ]]
		then
			# 1v1 or Fanhat
			sudo touch $FLAGFILEV1
			sudo chmod 666 $FLAGFILEV1
			echo "true" > $FLAGFILEV1
		else
...
argon_time_error() {
    ...
    echo " curl -k $ARGONDOWNLOADSERVER/tools/setntpserver.sh | bash"
}
...
then
	...
	sudo wget $ARGONDOWNLOADSERVER/$imagefile -O /usr/share/pixmaps/$imagefile --quiet
